    ...
    steps:
      ...
      - name: Run sync
        env:
          ...
          CLICKUP_LIST_URL:   ${{{{ secrets.CLICKUP_LIST_URL }}}}
          DATE_PRESET:        ${{{{ secrets.DATE_PRESET }}}}
          MATCH_PREFIX:       ${{{{ secrets.MATCH_PREFIX }}}}
          LOG_LEVEL:          INFO
...
def render_workflow_yaml(cron: str) -> bytes:
    return WORKFLOW_TEMPLATE.format(cron=cron).encode("utf-8")

# --------------------------------------------------------------------------- #
# High-level orchestration called from main.py
# --------------------------------------------------------------------------- #

def provision_schedule(cfg: dict[str, Any], sync_engine_source: dict[str, bytes]) -> dict[str, Any]:
    """
    End-to-end: ensure repo, push sync code, upload secrets, write workflow.
    Idempotent — safe to call repeatedly to update schedule/secrets.
    """
    ...
    secrets_to_push = {
        ...
        "CLICKUP_API_KEY":    cfg.get("clickup_api_key", ""),
        "CLICKUP_LIST_URL":   cfg.get("clickup_list_url", ""),
        "DATE_PRESET":        cfg.get("date_preset", "maximum"),
        "MATCH_PREFIX":       cfg.get("match_prefix", "Ad"),
    }
    ...
    # 5. Write the workflow YAML with the correct cron
    cron = build_cron(
    ...
    wf_result = put_file(
        ...
        render_workflow_yaml(cron),
        f"Schedule: {cfg.get('schedule_frequency')} @ {cfg.get('schedule_time')} {cfg.get('timezone')}",
    )
